...
{
  const rows: { Input: string; Chars: string; Mean: string; Min: string; Max: string; "ops/s": string; Throughput: string }[] = [];
  for (const [name, input] of Object.entries(ESCAPE_INPUTS)) {
    const r = bench(`escapeXml(${name})`, () => escapeXml(input));
    rows.push({
      Input: name, Chars: String(input.length),
      Mean: fmt(r.mean_ns), Min: fmt(r.min_ns), Max: fmt(r.max_ns),
      "ops/s": opsPerSec(r.mean_ns), Throughput: throughput(r.mean_ns, input.length),
    });
  }
  // @ts-expect-error Bun.inspect.table accepts options as third arg
  console.log(Bun.inspect.table(rows, ["Input", "Chars", "Mean", "Min", "Max", "ops/s", "Throughput"], { colors: useColor }));
}
...
{
  const rows: { Input: string; Chars: string; "Escape (mean)": string; "Decode (mean)": string; "Ratio (d/e)": string; "Round-trip": string }[] = [];
  for (const [name, input] of Object.entries(ESCAPE_INPUTS)) {
    const escaped = escapeXml(input);
    ...
    rows.push({
      Input: name, Chars: String(input.length),
      "Escape (mean)": fmt(re.mean_ns), "Decode (mean)": fmt(rd.mean_ns),
      "Ratio (d/e)": `${ratio.toFixed(2)}x`,
      "Round-trip": roundTrip ? "pass" : "FAIL",
    });
  }
  // @ts-expect-error Bun.inspect.table accepts options as third arg
  console.log(Bun.inspect.table(rows, ["Input", "Chars", "Escape (mean)", "Decode (mean)", "Ratio (d/e)", "Round-trip"], { colors: useColor }));
}
...
{
  const rows: { Items: string; Bytes: string; Mean: string; Min: string; Max: string; "ops/s": string; Throughput: string; "Per item": string }[] = [];
  for (const count of [1, 10, 50, 100]) {
    ...
    rows.push({
      Items: String(count), Bytes: String(sample.length),
      Mean: fmt(r.mean_ns), Min: fmt(r.min_ns), Max: fmt(r.max_ns),
      "ops/s": opsPerSec(r.mean_ns), Throughput: throughput(r.mean_ns, sample.length),
      "Per item": fmt(r.mean_ns / count),
    });
  }
  // @ts-expect-error Bun.inspect.table accepts options as third arg
  console.log(Bun.inspect.table(rows, ["Items", "Bytes", "Mean", "Min", "Max", "ops/s", "Throughput", "Per item"], { colors: useColor }));
}
...

{
  ...
  for (const count of [1, 10, 50, 100]) {
    ...
    rows.push({
      Items: String(count), Bytes: String(xml.length),
      Mean: fmt(r.mean_ns), Min: fmt(r.min_ns), Max: fmt(r.max_ns),
      "ops/s": opsPerSec(r.mean_ns), Throughput: throughput(r.mean_ns, xml.length),
      "Per item": fmt(r.mean_ns / count),
    });
  }
  // @ts-expect-error Bun.inspect.table accepts options as third arg
...

{
  ...
  for (const count of [10, 50, 100]) {
    ...
    rows.push({
      ...
      Faster: ratio > 1.05 ? `Atom (${ratio.toFixed(2)}x)` : ratio < 0.95 ? `RSS (${(1 / ratio).toFixed(2)}x)` : "~equal",
    });
  }
  // @ts-expect-error Bun.inspect.table accepts options as third arg
...
// Correctness
{
  ...
  const special = generateRssXml({
    title: "Test", description: "Test", link: "https://example.com",
...
{
  const rows: { Items: string; Bytes: string; "Gen (mean)": string; "Parse (mean)": string; "Total (mean)": string; "ops/s": string; "Per item": string }[] = [];
  for (const count of [1, 10, 50, 100]) {
    const channel = makeRssChannel(count);
    const sample = generateRssXml(channel);
    ...
    rows.push({
      Items: String(count), Bytes: String(sample.length),
      "Gen (mean)": fmt(rg.mean_ns), "Parse (mean)": fmt(rp.mean_ns),
      "Total (mean)": fmt(rt.mean_ns),
      "ops/s": opsPerSec(rt.mean_ns), "Per item": fmt(rt.mean_ns / count),
    });
  }
  // @ts-expect-error Bun.inspect.table accepts options as third arg
