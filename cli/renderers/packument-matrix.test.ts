...
describe("formatPerson", () => {
  ...
  test("object input with name and email", () => {
    expect(formatPerson({ name: "Alice", email: "alice@example.com" })).toBe(
  ...
  test("object input with name, email, and url", () => {
    expect(
      formatPerson({ name: "Alice", email: "a@b.com", url: "https://alice.dev" })
    ).toBe("Alice <a@b.com> (https://alice.dev)");
  });
