...
const MCP_BUN_DOCS_ROOT = join(BUN_PLATFORM_HOME, "matrix-analysis", "mcp-bun-docs");
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (m) {
        ...
        if (constant.name.includes("SECRET") || constant.name.includes("PRIVATE") || constant.name.includes("TOKEN")) {
...
function collectConstants(): ConstantMatch[] {
  ...
  try {
    if (statSync(MCP_BUN_DOCS_ROOT).isDirectory()) {
      for (const fp of walkTsFiles(MCP_BUN_DOCS_ROOT, MCP_BUN_DOCS_ROOT)) {
        matches.push(...extractFromFile(fp, "mcp-bun-docs", MCP_BUN_DOCS_ROOT));
      }
    }
  } catch (_) {}
  return matches;
...
function updateRegistry(matches: ConstantMatch[]): void {
  try {
    ...
    console.warn(`⚠️  Failed to update registry: ${error.message}`);
  }
}
...
function format95ColumnMatrix(matches: ConstantMatch[]): void {
  ...
  const header = "│ Constant".padEnd(20) + " │ Value".padEnd(25) + " │ Type".padEnd(10) + " │ Category".padEnd(12) + " │ Project │";
  const separator = "├".padEnd(21, "─") + "┼".padEnd(26, "─") + "┼".padEnd(11, "─") + "┼".padEnd(13, "─") + "┼".padEnd(9, "─") + "┤";
  
  console.log("┌" + "─".repeat(95) + "┐");
  console.log(header);
  console.log(separator);
  
  for (const match of matches.slice(0, 10)) { // Show first 10 for demo
...
function main(): void {
  ...
  for (const m of byProject["mcp-bun-docs"]) {
    ...
    const type = m.type || "string";
    const category = m.category || "config";
    const mcp = m.tier1380?.mcpExposed ? "✅" : "❌";
  ...
  console.log(`bun -e 'const v=await Bun.file("${REGISTRY_FILE}").json(); console.log(\`✅ v\${v.version} | Bun \${v.bunVersion} | Schema \${v.schemaVersion} | MCP \${v.mcpEnabled?"✓":"✗"}\`)'`);
  console.log("");
